...
<template>
  <div class="page-wrap">
    ...
    <div v-if="!store.loading && !store.tasks.length" class="empty-state"><Radio :size="28" /><h2>工作台尚无任务数据</h2><p>创建第一个任务后，这里会显示真实进度、风险与成员工作量。</p><button class="btn-primary" @click="store.openTask()">新建任务</button></div>
    <template v-else>
    <section class="grid gap-px overflow-hidden rounded-xl border border-line bg-line sm:grid-cols-2 xl:grid-cols-4">
      <article v-for="stat in stats" :key="stat.label" class="bg-panel p-5">
        ...
        <p class="mt-2 font-mono text-[10px] text-muted">{{ stat.delta }}</p>
      </article>
    </section>
    <section class="mt-5 grid gap-5 xl:grid-cols-[1.55fr_.8fr]">
      <article class="card">
        ...
        <div class="mt-4 flex justify-between font-mono text-[9px] text-muted"><span>0%</span><span>{{ progress }}%</span><span>100%</span></div>
