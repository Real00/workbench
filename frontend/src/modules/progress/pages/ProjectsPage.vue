...
<template>
  <div class="page-wrap">
    ...
    <div v-if="!store.loading && !store.projects.length" class="empty-state"><FolderKanban :size="28" /><h2>尚无项目</h2><p>立项后即可把相关任务挂到项目下，任务也可以不关联项目。</p><button class="btn-primary" @click="store.openProject()">新增项目</button></div>
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <button v-for="item in projectCards" :key="item.project.id" class="card group text-left" @click="store.openProject(item.project)">
        ...
        <p class="mt-3 font-mono text-[10px] text-muted">立项 {{ formatDate(item.project.started_at) }}</p>
        <p v-if="item.project.description" class="mt-2 line-clamp-2 text-xs leading-5 text-slate-300">{{ item.project.description }}</p>
        <p v-if="item.project.background" class="mt-2 line-clamp-2 text-[11px] leading-5 text-slate-400">{{ item.project.background }}</p>
        ...
        <div class="flex justify-between font-mono text-[10px] text-muted"><span>{{ item.taskCount }} 任务 · {{ item.activeCount }} 进行中</span><span>{{ item.taskCount ? `${Math.round(item.taskCount / store.tasks.length * 100)}% 占比` : '暂无关联任务' }}</span></div>
      </button>
    </section>
  </div>
</template>
