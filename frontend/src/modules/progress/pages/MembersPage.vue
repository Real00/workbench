...
<template>
  <div class="page-wrap">
    ...
    <section v-else class="entity-grid grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <button v-for="member in store.members" :key="member.id" class="card entity-card group text-left" @click="store.openMember(member)">
        <div class="relative member-portrait">
          ...
          <span class="absolute top-2 right-2 flex gap-1.5">
            ...
            <span :class="['status-chip', !member.active && 'status-chip--inactive']">{{ member.active ? '可分配' : '停用' }}</span>
          </span>
        </div>
        ...
        <div class="mt-4 flex justify-between font-mono text-[11px] text-muted"><span>{{ workloads.get(member.id)?.current_tasks.length ?? 0 }} 个未完成任务</span><span>{{ member.evaluations?.length ?? 0 }} 条评价</span><span>{{ workloads.get(member.id)?.estimated_remaining_days ?? 0 }}d 剩余</span></div></div>
      </button>
    </section>
    <section v-if="store.members.length" class="mt-5">
      <article class="card"><div class="card-head"><div><p class="eyebrow">Completion</p><h2>成员任务完成度</h2></div><ShieldCheck :size="17" class="text-cyan" /></div>
